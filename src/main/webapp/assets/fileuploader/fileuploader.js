...
qq.FileUploaderBasic = function(o){
    ...
    this._options = {
        ...
        debug: false,
        action: '/components/fileupload/upload',
        deleteUrl: '/components/fileupload/delete/',
...
qq.FileUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        fileTemplate: '<li>' +
            '<input type="hidden" class="qq-upload-fileId" name="'+this._options.fileIdInputName+'" value="" />' +
    ...
    if(this._options.files && this._options.files.length>0){
    	for(var i=0;i<this._options.files.length;i++){
    		var fileInfo = this._options.files[i];
    		this._addExistFileToList(fileInfo.fileId,fileInfo.fileName);
    	}
    }
};
...
qq.extend(qq.FileUploader.prototype, {
     ...
     * Gets one of the elements listed in this._options.classes
     **/
    _findAll: function(parent, type){
        var element = qq.getByClass(parent, this._options.classes[type]);
        return element;
    },
    ...
    _onComplete: function(id, fileName, result){
        ...
        if (result.success){
            ...
            this._find(item, 'del').style.display = "inline-block";
            this._find(item, 'fileId').value = result.fileId;
    ...
    _addExistFileToList: function(fileId, fileName){
        var item = qq.toElement(this._options.fileTemplate);
        ...
        qq.addClass(item, this._classes.success);
        this._find(item, 'del').style.display = "inline-block";
        this._find(item, 'fileId').value =fileId;
        if (this._classes.successIcon) {
            this._find(item, 'finished').style.display = "inline-block";
            qq.addClass(item, this._classes.successIcon);
        }
    ...
    _clearList: function(){
    	var fileIdInputs = this._findAll(this._listElement, "fileId");
    	if(fileIdInputs && fileIdInputs.length>0){
    		for(var i=0;i<fileIdInputs.length;i++){
           	 	qq.AjaxUpdate(this._options.deleteUrl + fileIdInputs[i].value);
    		}
    	}
        this._listElement.innerHTML = '';
    ...
    _bindDeleteEvent: function(){
        ...
        this._attach(list, 'click', function(e){
            ...
            if (qq.hasClass(target, self._classes.del)){
                qq.preventDefault(e);
                var item = target.parentNode;
                var fileIdInput = self._find(item, "fileId");
                if(fileIdInput){
                	var url = self._options.deleteUrl + fileIdInput.value;
                	 qq.AjaxUpdate(url);
                }
                qq.remove(item);
            }
...
/**
 * //recvType:格式 url：地址 method：方式 recvFunc：处理结果函式
 * @param recvType
 * @param url
 * @param str
 * @param method
 * @param recvFunc
 */
qq.AjaxUpdate=function(url,data,method,type,callback){
	var request = false;
    ...
    if(callback){
	    if(type="XML" || type=="xml"){
	 	   request.onreadystatechange = function (){callback(request.responseXML);};
	    }else{
